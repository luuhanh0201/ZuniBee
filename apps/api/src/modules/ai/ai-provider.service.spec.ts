...
describe('AiProviderService', () => {
  ...
  it('runs an inference probe and persists the measured health', async () => {
    const row = provider();
    const repository = {
    ...
    const sdk = {
      listModels: jest.fn().mockResolvedValue([{ id: row.model }]),
    ...
    expect(sdk.listModels).toHaveBeenCalledWith(
      expect.objectContaining({
        driver: 'ollama',
        baseUrl: row.baseUrl,
      }),
    ...
    expect(sdk.generate).toHaveBeenCalledWith(
      expect.objectContaining({
        provider: expect.objectContaining({ model: row.model }),
        maxOutputTokens: 32,
  ...
  it('persists an offline result instead of returning fake success', async () => {
    ...
    const sdk = {
      listModels: jest
        .fn()
        .mockRejectedValue(
          new AiProviderSdkError('model endpoint missing', 404),
        ),
      generate: jest.fn(),
    };
    const service = new AiProviderService(
  ...
  it('reports a Gemini inference outage instead of treating model discovery as healthy', async () => {
    const row = provider({
      ...
      generate: jest
        ...
        .mockRejectedValue(
          new AiProviderSdkError(
            'Retryable HTTP Error: Service Unavailable',
            null,
          ),
        ),
    };
    const service = new AiProviderService(
      ...
      countRepository() as never,
      {} as never,
      { decrypt: jest.fn().mockReturnValue('gemini-key') } as never,
      { assertAllowed: jest.fn().mockResolvedValue(undefined) } as never,
      sdk as never,
    );

    await expect(service.testConnection(row.id)).resolves.toMatchObject({
      ok: false,
      message:
        'Provider không xử lý inference: Retryable HTTP Error: Service Unavailable',
      provider: {
        healthStatus: AiProviderHealthStatus.OFFLINE,
        lastHealthError:
          'Provider không xử lý inference: Retryable HTTP Error: Service Unavailable',
      },
    });
  ...
  it('discovers models from an OpenAI-compatible provider with bearer auth', async () => {
    ...
    const sdk = {
      listModels: jest
        .fn()
        .mockResolvedValue([{ id: 'gpt-5-mini' }, { id: 'gpt-4.1-mini' }]),
    };
    const service = new AiProviderService(
      {} as never,
      {} as never,
  ...
  it('reports an Anthropic 401 as an API key error', async () => {
    const sdk = {
      listModels: jest
        .fn()
        .mockRejectedValue(new AiProviderSdkError('unauthorized', 401)),
    };
    const service = new AiProviderService(
      {} as never,
      {} as never,
  ...
  it('discovers locally installed Ollama models from api/tags', async () => {
    ...
    const sdk = {
      listModels: jest
        .fn()
        .mockResolvedValue([{ id: 'llama3.1:8b' }, { id: 'qwen3:8b' }]),
    };
    const service = new AiProviderService(
      {} as never,
      {} as never,
    ...
    expect(sdk.listModels).toHaveBeenCalledWith(
      expect.objectContaining({ driver: 'ollama' }),
    );
  });
  ...
  it('tests an unsaved provider configuration without persisting it', async () => {
    const policy = { assertAllowed: jest.fn().mockResolvedValue(undefined) };
    const sdk = {
      listModels: jest.fn().mockResolvedValue([{ id: 'gpt-5-mini' }]),
      generate: jest.fn().mockResolvedValue({
        content: 'OK',
        refused: false,
        truncated: false,
    ...
    expect(sdk.listModels).toHaveBeenCalledWith(
      expect.objectContaining({ driver: 'openai', apiKey: 'secret-key' }),
    );
    expect(sdk.generate).toHaveBeenCalledWith(
      expect.objectContaining({
        provider: expect.objectContaining({ model: 'gpt-5-mini' }),
        maxOutputTokens: 32,
      }),
    );
